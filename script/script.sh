...
if [ ! -f "/etc/smbcredentials/$2.cred" ]; then
    sudo bash -c 'echo "username=$2" >> /etc/smbcredentials/$2.cred'
    sudo bash -c 'echo "password=$3" >> /etc/smbcredentials/$2.cred'
fi
sudo chmod 600 /etc/smbcredentials/$2.cred
sudo bash -c 'echo "//$2.file.core.windows.net/wp-content /var/www/html/wp-content cifs nofail,vers=3.0,credentials=/etc/smbcredentials/$2.cred,dir_mode=0777,file_mode=0777,serverino" >> /etc/fstab'
